function compareTau(path_root, DIV1, DIV2, DIV3, DIV4, yLimMax)
% Ver 2.0
...
for si = 1:sl
    ...
    for i=1:subplotsx
        for ii=1:subplotsy
            ...
            if i==1
                ylabel(['Glycerol ',num2str(round(glycerol(ii)*10)/10),' v/v'])
            end
            
            if ii==1
                xlabel(['RNA ',num2str(round(rna(i)*10)/10),' ug/mL'])
            end

            
            subbody = body(si == saltindex & ri == rnaIndex & gi == glycerolIndex, :);
            if size(subbody, 1) == 0
                continue
            end
            %% sort subbody
