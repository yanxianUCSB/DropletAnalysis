function compareThisPolyA(path_root, DIV1, DIV2, DIV3, DIV4, Pick, yLimMax)
% Ver 16041501
...
for si = 1:sl
    ...
    for i=1:subplotsx
        for ii=1:subplotsy
            ...
            ti = find(tau == Pick(2));
            gi = find(glycerol == Pick(4));
            
            if ii==subplotsy
                title(['NaCl ', num2str(round(Pick(1)*10)/10),' mM'])
            end
            ...
            subbody = body(find(salt == Pick(1)) == saltindex & ti == tauIndex & gi == glycerolIndex, :);
            if size(subbody, 1) == 0
                continue
            end
            %% sort subbody
