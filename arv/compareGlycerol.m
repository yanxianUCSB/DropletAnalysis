function compareGlycerol(path_root, DIV1, DIV2, DIV3, DIV4, yLimMax)
% Ver 3.0


analtype = 'compareGlycerol';
...
for ri = 1:rl
    ...
    for xi=1:subplotsx
        for yi=1:subplotsy
            ...
            if xi==1
                ylabel(['Salt ',num2str(round(salt(si)*10)/10),' mM'])
            end
            ...
            subbody = body(si == saltindex & ti == tauIndex & ri == rnaIndex, :);
            if size(subbody, 1) == 0
                continue
            end
            %% sort subbody
